...
practice_data1 = 
  practice_data %>%
  select(employee_id, age, hospital_visit, salary, health_score)
```

```{r}
nested_data = nest(practice_data1, data = age:health_score)
```
...
```{r}
lm_func = function(df) {
  lm(health_score ~ age, data = df)
}
```

```{r}
lm_func(df = nested_data$data[[1]])
```

```{r}
#map(nested_data$data, lm_func)
```
