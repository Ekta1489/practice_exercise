...
dat_csv %>%
  ...
  ) + geom_bar(width = 0.5, fill = "lightblue", position = "dodge") 
```
...
dat_csv %>%
  ...
  ) + geom_bar(width = 0.5, fill = "pink", position = "dodge") 
```
...
```{r}
age_dis %>%
  ggplot(aes(x = age_cat, y = freq, fill = hcc_at_5_years)) + geom_bar(stat = "identity") + scale_x_discrete(name = "Age Category", limits = c("1","2","3")) + 
...
```
```{r}
ggplot(hcc_alc, aes(x = age_cat, y = alc)) +
  geom_bar(aes(fill = hcc_at_5_years), position = "dodge", stat="identity", width=.5) +
      geom_text(aes(label = alc, group=hcc_at_5_years), position=position_dodge(width=0.5), vjust = -0.5) +
